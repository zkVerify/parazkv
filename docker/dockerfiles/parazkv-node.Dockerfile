...
WORKDIR /usr/src/parazkv-node
COPY . .

# Build the node
RUN cargo build -p parazkv-node --profile "${PROFILE}" --features "${FEATURES}"

FROM ubuntu:24.04 AS node

SHELL ["/bin/bash", "-c"]

ARG BINARY="parazkv-node"
ARG DESCRIPTION="Parazkv Parachain"
...
COPY --from=builder "/usr/src/parazkv-node/target/${PROFILE}/${BINARY}" "/usr/local/bin/"
COPY --from=builder "/usr/src/parazkv-node/target/${PROFILE}/wbuild/parazkv-runtime/parazkv_runtime.compact.compressed.wasm" "./parazkv_runtime.compact.compressed.wasm"
