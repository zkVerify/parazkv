...
const AFTER_HELP_EXAMPLE: &str = color_print::cstr!(
   ...
   <bold>parazkv-node build-spec --disable-default-bootnode > plain-parachain-chainspec.json</>
           Export a chainspec for a local testnet in json format.
   <bold>parazkv-node --chain plain-parachain-chainspec.json --tmp -- --chain local</>
           Launch a full node with chain specification loaded from plain-parachain-chainspec.json.
   <bold>parazkv-node</>
           Launch a full node with default parachain <italic>local-testnet</> and relay chain <italic>local</>.
   <bold>parazkv-node --collator</>
