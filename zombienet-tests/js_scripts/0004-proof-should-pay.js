...
const { PROOF, PUBS, VK } = require('./groth16_data.js');
...
async function run(nodeName, networkInfo, _args) {
    ...
    if (!receivedEvents(await submitProof(api.tx.settlementGroth16Pallet, alice, { 'Vk': VK }, PROOF, PUBS))) {
    ...
    if (receivedEvents(await submitProof(api.tx.settlementGroth16Pallet, alice, { 'Vk': VK }, PROOF, 0))) {
