...
async function run(nodeName, networkInfo, args) {
    ...
    let init_balance_receiver = (await api.query.system.account(decodeAddress(receiver)))["data"]["free"];
    let balance_receiver = init_balance_receiver;

    while (!balance_receiver.eq(new BN(amount, 10))) {
        await new Promise(r => setTimeout(r, 1000));
        timeout -= 1000;
        balance_receiver = (await api.query.system.account(decodeAddress(receiver)))["data"]["free"];
        if (timeout <= 0) {
